...
import { applyPipeline } from '@/core/transformations/applyPipeline';
...
function PipelineContent() {
  ...
  const handleSaveToHistory = async () => {
    ...
    const customName = await customPrompt("Como deseja nomear este dataset limpo?", defaultName);
    ...
    const choice = await customChoice(`Dataset "${newDataset.name}" salvo no Histórico com sucesso! O que deseja fazer agora?`, [
      { label: 'Continue Editing', value: 'stay' },
      { label: 'Go to History', value: 'history' },
      { label: 'Go to Merge ⋈', value: 'merge', isPrimary: true }
    ]);
  ...
  return (
    <div className="container" style={{ display: 'flex', gap: '2rem', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>Visual ETL Pipeline</h2>
          <p style={{ color: '#94a3b8' }}>Apply non-destructive transformations to <strong>{dataset.name}</strong></p>
        </div>
        <div style={{ display: 'flex', gap: '1rem' }}>
          ...
          <button className="btn btn-secondary" onClick={handleSaveToHistory} disabled={isSavingHistory} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--primary)', borderColor: 'var(--primary)' }}>
            <Save size={18} /> {isSavingHistory ? 'Saving...' : 'Save to History'}
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '2rem', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="glass" style={{ padding: '1.5rem', borderRadius: 'var(--radius-lg)', display: 'flex', gap: '2rem' }}>
            <div>
              <span style={{ color: '#94a3b8', display: 'block', fontSize: '0.875rem' }}>Original Rows</span>
              <span style={{ fontSize: '1.5rem', fontWeight: 600 }}>{dataset.rows.length}</span>
            </div>
            <div>
              <span style={{ color: '#94a3b8', display: 'block', fontSize: '0.875rem' }}>Rows After Pipeline</span>
            ...
            <div>
              <span style={{ color: '#94a3b8', display: 'block', fontSize: '0.875rem' }}>Columns</span>
